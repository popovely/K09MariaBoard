...
public class CookieManager {
	/* 
	쿠키생성
		: 생성시 response 내장객체가 있어야하므로
		  HttpServletResponse 타입의 매개변수로 받아야함. 
	 */
	public static void makeCookie(HttpServletResponse response,
			String cName, String cValue, int cTime) {
		Cookie cookie = new Cookie(cName, cValue);// 생성자를 통해 쿠키 생성(쿠키명, 쿠키
		cookie.setPath("/");					// 경로설정
	...
	쿠키값 읽기
		: request 내장객체가 필요하므로
		  HttpServletRequest 타입의 매개변수로 받아야 함.
	 */
	public static String readCookie(HttpServletRequest request, String cName) {
		...
		// 일치하는 쿠키명이 없으면 cookieValue를 선언할 때의 값이 유지됨 ;빈값
	}
	
	// 쿠키 삭제
	public static void deleteCookie(HttpServletResponse response, String cName) {
		/*
			쿠키는 삭제를 위한 메소드가 별도로 없고,
			빈값과 유효기간을 0으로 설정하면 삭제된다.
			따라서 앞에서 생성한 makeCookie() 메소드를 재활용한다.
		 */
